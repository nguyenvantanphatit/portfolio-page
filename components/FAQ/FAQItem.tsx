...
const FAQItem = ({ faqData }: { faqData: FaqData }) => {
  ...
  return (
    <>
      <div className="flex flex-col border-b border-stroke pt-8 last-of-type:border-none dark:border-strokedark">
        <button
          onClick={() => {
            handleFaqToggle(id);
          }}
          className="flex cursor-pointer items-center justify-between px-6 py-4 text-metatitle3 font-medium text-black "
        >
          {quest}
          {activeFaq === id ? (
            <svg
        ...
        <p
          className={`border-t border-stroke px-6 py-4 dark:border-strokedark ${
